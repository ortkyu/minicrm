...
export const MainBuilding = ({build, places, setId, setActiv, activElement, inventory }) => {
    

    const[openpartsOfBuilding, setOpen] = useState(false)
    const togle = () => setOpen(!openpartsOfBuilding)

    let partsOfBuilding = build && places.filter(i=> build.parts.includes(i.id))

    let childConstructionsParts = partsOfBuilding.flatMap(c=>c.parts)
    let idsByPartsBuild = partsOfBuilding && places.filter(i => childConstructionsParts.includes(i.id)).flatMap(p=>p.id)


const addId = () => {
    idsByPartsBuild ? setId([build.id, ...build.parts, ...idsByPartsBuild])
...
let inventoryAvailability = idsInvent.filter(i=> idsChild.includes(i) || partsChild.includes(i) )


    return (
        <div className="wrap">
                <span className="icon" onClick={togle}>
                <svg stroke-width="10" stroke="#2F3435" height="17px" id="Layer_1"  version="1.1" viewBox="0 0 128 128" width="22px"  xmlns="http://www.w3.org/2000/svg" >
                        <line  x1="111" x2="64" y1="40.5" y2="87.499"/>
                        <line  x1="64" x2="17" y1="87.499" y2="40.5"/>
                </svg>
                </span>
                <span className={build.id === activElement ? "activ" : "name"} onClick={()=>addId(build.id)}>
                    <span>
                    {build.data.name}
                    </span>
                    </span>
                { inventoryAvailability.length > 0 && <span  className="iconInv">
                ...
                { openpartsOfBuilding && <div>
                    {partsOfBuilding.map(construction=>
                    <PartOfBuilding onClick={(e)=>console.log(e.event.target.key)} inventory={inventory}  activElement={activElement} key={construction.id} setActiv={setActiv} construction={construction} setId={setId} places={places}/>
                    )}
                </div>}
        </div>
    )
