...
export const Room = ({ room, activElement, setId, setActiv, inventory }) => {
    
    let inventoryAvailability = inventory.filter(i=> i.placeId === room.id)

    return (
        <div>
                    <span className={room.id === activElement ? "activ" : "name"} onClick = {()=>{setId(room.id); setActiv(room.id)} }  key={room.id} >
                    <span>
                        {room.data.name}
                        </span>
                        </span>
