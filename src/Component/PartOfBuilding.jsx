...
export const PartOfBuilding = ({construction, places, setId, setActiv, activElement, inventory }) => {
    

    let rooms = construction.parts && places.filter(i=>construction.parts.includes(i.id))
...
let inventoryAvailability = inventory.filter(i=> i.placeId === construction.id || (construction.parts && construction.parts.includes(i.placeId)))


    return (
        <div>
                { construction.parts && <span className="icon" onClick={togle}>
                ...
                <span className={construction.id === activElement ? "activ" : "name"} onClick={()=>addId()}>
                <span>
               ...
               { openChildParts &&<div>
                    ...
                    <Room className={room.id === activElement && "activ"} onClick = {()=>{setId(room.id); setActiv(room.id)} } room={room} setId={setId} setActiv={setActiv} activElement={activElement} inventory={inventory}  key={room.id} >{room.data.name}</Room>
                    )}
                </div>}
        </div>
    )
